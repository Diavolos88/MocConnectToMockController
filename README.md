...
mock-controller:
  ...
  healthcheck-timeout-seconds: 5  # Таймаут для healthcheck (в секундах)
  healthcheck-interval-seconds: 10  # Интервал проверки healthcheck (в секундах)
  connect-timeout-seconds: 10  # Таймаут подключения (в секундах)
  read-timeout-seconds: 10  # Таймаут чтения ответа (в секундах)
...
1. **При старте приложения:**
   ...
   - Собирает конфигурацию от всех сервисов (поля с префиксами `delay*`, `int*`, `is*`, `string*`)
...
3. **Применение конфигурации:**
   ...
   - Поддерживаются типы: `long`, `int`, `boolean`, `String`
...
- Проверьте логи на наличие предупреждений о невалидных значениях
- Убедитесь, что значения соответствуют типу поля:
  - Для `int` поля: значение должно быть в диапазоне от `-2147483648` до `2147483647`
  - Для `long` поля: значение должно быть в диапазоне от `-9223372036854775808` до `9223372036854775807`
  - Для `boolean` поля: значение должно быть `"true"` или `"false"`
- Невалидные значения пропускаются, но остальные значения применяются нормально
- Поле сохраняет свое текущее значение, если новое значение невалидно
